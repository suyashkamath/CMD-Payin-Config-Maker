...
vehicle_types    = load_json(os.path.join(BASE, 'vehicle_type.json'))
rto_list         = load_json(os.path.join(BASE, 'rto_id_name.json'))
location_mapping = load_json(os.path.join(BASE, 'location_mapping.json'))
...
# ─── RTO lookup with location_mapping fallback ───────────────────────────────
def get_rto(geo_new):
    """Returns (rto_group_id, rto_group_name)
    Priority:
      1. Direct match in rto_dict
      2. Fallback via location_mapping.json (take first valid match)
      3. Return -1 if nothing found
    """
    # 1. Direct match
    if geo_new in rto_dict:
        return rto_dict[geo_new], geo_new

    # 2. Fallback via location_mapping
    if geo_new in location_mapping:
        mapped = location_mapping[geo_new]
        if isinstance(mapped, list):
            for m in mapped:
                if m in rto_dict:
                    return rto_dict[m], m
        else:
            if mapped in rto_dict:
                return rto_dict[mapped], mapped
...
for idx, row in df.iterrows():
    ...
    rto_group_id, rto_group_name = get_rto(geo_new)
